local M = {
  id = "CommonsBase_Remote.GitHub@0.1.1"
}
...
CommonsBase_Remote__GitHub__0_1_1 = {}
...
function CommonsBase_Remote__GitHub__0_1_1.parse_positive_int(name, value, default)
  if value == nil then
    return default
  end
...
function CommonsBase_Remote__GitHub__0_1_1.user_scalar(value)
...
function CommonsBase_Remote__GitHub__0_1_1.parse_timestamp(value)
  if value == nil then
...
function CommonsBase_Remote__GitHub__0_1_1.normalize_repo(repo)
...
function CommonsBase_Remote__GitHub__0_1_1.parse_common_args(request)
  local p = {}
  local continued = request.continued or {}
  local create_repo = CommonsBase_Remote__GitHub__0_1_1.user_scalar(request.user.create_repo)
  local dry_run = CommonsBase_Remote__GitHub__0_1_1.user_scalar(request.user.dry_run)
  if create_repo == nil then
    create_repo = CommonsBase_Remote__GitHub__0_1_1.user_scalar(continued.create_repo)
  end
  if dry_run == nil then
    dry_run = CommonsBase_Remote__GitHub__0_1_1.user_scalar(continued.dry_run)
  end
  local timestamp = CommonsBase_Remote__GitHub__0_1_1.user_scalar(request.user.timestamp)
  p.workspace = assert(stringdk.sanitizesubpath(CommonsBase_Remote__GitHub__0_1_1.user_scalar(request.user.workspace) or "dk.u"), "Expected `workspace=RELATIVE_PATH`")
  p.sessions = CommonsBase_Remote__GitHub__0_1_1.parse_positive_int("sessions", CommonsBase_Remote__GitHub__0_1_1.user_scalar(request.user.sessions), 4)
  p.retention = CommonsBase_Remote__GitHub__0_1_1.parse_positive_int("retention", CommonsBase_Remote__GitHub__0_1_1.user_scalar(request.user.retention), 8)
  ...
  p.timestamp = CommonsBase_Remote__GitHub__0_1_1.parse_timestamp(timestamp)
  p.repo = CommonsBase_Remote__GitHub__0_1_1.normalize_repo(CommonsBase_Remote__GitHub__0_1_1.user_scalar(request.user.repo) or "")
  p.cmd = assert(CommonsBase_Remote__GitHub__0_1_1.user_scalar(request.user.cmd), "Expected `cmd=COMMAND`")
  p.commandvsl = assert(CommonsBase_Remote__GitHub__0_1_1.user_scalar(request.user.commandvsl), "Expected `commandvsl=COMMAND`")
  ...
  p.requested_trace_key_id = CommonsBase_Remote__GitHub__0_1_1.user_scalar(request.user.requested_trace_key_id)
  p.requested_trace_key_description = CommonsBase_Remote__GitHub__0_1_1.user_scalar(request.user.requested_trace_key_description)
  local project_root = CommonsBase_Remote__GitHub__0_1_1.user_scalar(request.user.project_root)
  if project_root == nil then
    project_root = CommonsBase_Remote__GitHub__0_1_1.user_scalar(continued.project_root)
  end
  ...
  local gh_user = CommonsBase_Remote__GitHub__0_1_1.user_scalar(request.user.gh)
  p.gh = gh_user or "gh"
  p.gh_user_supplied = false
  if gh_user then
    p.gh_user_supplied = true
  end
  p.git = CommonsBase_Remote__GitHub__0_1_1.user_scalar(request.user.git) or "git"
  return p
end

function CommonsBase_Remote__GitHub__0_1_1.make_dry_run_plan(p)
...
function CommonsBase_Remote__GitHub__0_1_1.write_plan(request, file, p, rule_name)
  local create_repo = "false"
  local dry_run = "false"
  local plan = CommonsBase_Remote__GitHub__0_1_1.make_dry_run_plan(p)
  ...
  request.io.write(file, "[dry-run] CommonsBase_Remote.GitHub." .. rule_name .. "@0.1.1\n")
  ...
  request.io.write(file, "workflow_generated_by=CommonsBase_Remote.GitHub@0.1.1\n")
...
function CommonsBase_Remote__GitHub__0_1_1.trim(s)
  s = s or ""
  local first = 1
...
function CommonsBase_Remote__GitHub__0_1_1.replace_all(s, needle, replacement)
  local out = {}
  local i = 1
...
function CommonsBase_Remote__GitHub__0_1_1.base64_encode(s)
...
function CommonsBase_Remote__GitHub__0_1_1.write_base64_lines(request, path, values)
  CommonsBase_Remote__GitHub__0_1_1.write_text(request, path, CommonsBase_Remote__GitHub__0_1_1.base64_lines_text(values))
end

function CommonsBase_Remote__GitHub__0_1_1.base64_lines_text(values)
  local out = {}
  local i = 1
  while values[i] do
    table.insert(out, CommonsBase_Remote__GitHub__0_1_1.base64_encode(values[i]))
    i = i + 1
  end
  return table.concat(out, "\n") .. "\n"
end

function CommonsBase_Remote__GitHub__0_1_1.lines(s)
  local t = {}
  s = s or ""
  local pos = 1
...
function CommonsBase_Remote__GitHub__0_1_1.first_line(s)
  local lines = CommonsBase_Remote__GitHub__0_1_1.lines(s)
  return lines[1] or ""
end

function CommonsBase_Remote__GitHub__0_1_1.shell_quote(s)
  if s == "" then
    return "''"
  end
  if string.find(s, "[^A-Za-z0-9_@%%+=:,./-]") then
    return "'" .. CommonsBase_Remote__GitHub__0_1_1.replace_all(s, "'", "'\\''") .. "'"
  end
  return s
end

function CommonsBase_Remote__GitHub__0_1_1.show_command(program, args)
  local parts = { program }
  local i = 1
  while args[i] do
    table.insert(parts, CommonsBase_Remote__GitHub__0_1_1.shell_quote(args[i]))
    i = i + 1
  end
  return table.concat(parts, " ")
end

function CommonsBase_Remote__GitHub__0_1_1.helper_envmods(options)
  local envmods = {}
...
function CommonsBase_Remote__GitHub__0_1_1.capture(request, program, args, options)
  options = options or {}
  local allowfailure = options.allowfailure or options.allow_failure
  print("+ " .. CommonsBase_Remote__GitHub__0_1_1.show_command(program, args))
  local request_options = {
    program = program,
    args = args,
    max_output_bytes = options.max_output_bytes or 16777211,
    envmods = CommonsBase_Remote__GitHub__0_1_1.helper_envmods(options),
  }
  if options.cwd then
...
function CommonsBase_Remote__GitHub__0_1_1.spawn(request, program, args, options)
  options = options or {}
  print("+ " .. CommonsBase_Remote__GitHub__0_1_1.show_command(program, args))
  local request_options = {
    program = program,
...
function CommonsBase_Remote__GitHub__0_1_1.path_join(base, child)
  local sep = "/"
...
function CommonsBase_Remote__GitHub__0_1_1.is_windows(request)
...
function CommonsBase_Remote__GitHub__0_1_1.write_text(request, path, content)
...
function CommonsBase_Remote__GitHub__0_1_1.dirname(path)
...
function CommonsBase_Remote__GitHub__0_1_1.windows_quote(s)
  s = tostring(s or "")
  if s == "" then
    return "\"\""
  end
...
function CommonsBase_Remote__GitHub__0_1_1.windows_force_quote(s)
  s = tostring(s or "")
  return "\"" .. CommonsBase_Remote__GitHub__0_1_1.replace_all(s, "\"", "\"\"") .. "\""
end

function CommonsBase_Remote__GitHub__0_1_1.try_capture(request, program, args, options)
  options = options or {}
  local request_options = {
    program = program,
    args = args,
...
function CommonsBase_Remote__GitHub__0_1_1.find_named_file_abs(request, dir, name)
  local entries = request.io.list(dir, "all")
  local i = 1
  while entries[i] do
    local entry = entries[i]
    if request.io.isdir(entry) then
      local nested = CommonsBase_Remote__GitHub__0_1_1.find_named_file_abs(request, entry, name)
    ...
    elseif request.io.isfile(entry) then
      ...
      if CommonsBase_Remote__GitHub__0_1_1.basename(rel) == name then
...
function CommonsBase_Remote__GitHub__0_1_1.project_root_cmd(request)
  local result = CommonsBase_Remote__GitHub__0_1_1.try_capture(
    request,
    "cmd",
    { "/d", "/c", "cd" },
  ...
  return CommonsBase_Remote__GitHub__0_1_1.trim(CommonsBase_Remote__GitHub__0_1_1.first_line(result.stdout))
end

function CommonsBase_Remote__GitHub__0_1_1.project_root_sh(request)
  local result = CommonsBase_Remote__GitHub__0_1_1.try_capture(
    request,
    "pwd",
    {},
  ...
  return CommonsBase_Remote__GitHub__0_1_1.trim(
    CommonsBase_Remote__GitHub__0_1_1.first_line(result.stdout))
end

function CommonsBase_Remote__GitHub__0_1_1.project_root_path(request)
  if CommonsBase_Remote__GitHub__0_1_1.is_windows(request) then
    return CommonsBase_Remote__GitHub__0_1_1.project_root_cmd(request)
  end
  return CommonsBase_Remote__GitHub__0_1_1.project_root_sh(request)
end

function CommonsBase_Remote__GitHub__0_1_1.normalize_program(program)
...
function CommonsBase_Remote__GitHub__0_1_1.local_dk0_program(request, snapshot_dir)
  if snapshot_dir then
    ...
    if CommonsBase_Remote__GitHub__0_1_1.is_windows(request) then
      return CommonsBase_Remote__GitHub__0_1_1.normalize_program(root .. "\\dk0.cmd")
    end
    return CommonsBase_Remote__GitHub__0_1_1.normalize_program(
      CommonsBase_Remote__GitHub__0_1_1.path_join(root, "dk0"))
  end
  local root = CommonsBase_Remote__GitHub__0_1_1.project_root_path(request)
  if root ~= "" then
    if CommonsBase_Remote__GitHub__0_1_1.is_windows(request) then
      return root .. "\\dk0.cmd"
    end
    return CommonsBase_Remote__GitHub__0_1_1.path_join(root, "dk0")
  end
  return "./dk0"
end

function CommonsBase_Remote__GitHub__0_1_1.run_local_dk0(request, snapshot_dir, args)
  if snapshot_dir then
    local snapshot_root = request.io.realpath(snapshot_dir)
    local snapshot_program = CommonsBase_Remote__GitHub__0_1_1.local_dk0_program(request, snapshot_dir)
    return CommonsBase_Remote__GitHub__0_1_1.capture(
      request,
      snapshot_program,
      args,
      { cwd = snapshot_root })
  ...
  if CommonsBase_Remote__GitHub__0_1_1.is_windows(request) then
    local root = CommonsBase_Remote__GitHub__0_1_1.project_root_cmd(request)
    if root ~= "" then
      local cmdline = ".\\dk0.cmd"
      local i = 1
      while args[i] do
        cmdline = cmdline .. " " .. CommonsBase_Remote__GitHub__0_1_1.windows_quote(args[i])
        i = i + 1
      end
      local root_result = CommonsBase_Remote__GitHub__0_1_1.try_capture(
        request,
        "cmd",
        { "/d", "/c", cmdline },
  ...
  local local_program = CommonsBase_Remote__GitHub__0_1_1.local_dk0_program(request, snapshot_dir)
  return CommonsBase_Remote__GitHub__0_1_1.capture(
    request,
    local_program,
    args,
...
function CommonsBase_Remote__GitHub__0_1_1.ensure_coreutils(request, snapshot_dir, p)
  local program = ".dk/r/c/.local/coreutils/coreutils.exe"
  local probe = CommonsBase_Remote__GitHub__0_1_1.try_capture(
    request,
    program,
    { "--version" },
  ...
  if probe.code ~= "0" then
    ...
    if project_root == "" then
      project_root = CommonsBase_Remote__GitHub__0_1_1.project_root_path(request)
    end
    if project_root ~= "" then
      if CommonsBase_Remote__GitHub__0_1_1.is_windows(request) then
        import_dir = project_root .. "\\etc\\dk\\i"
      else
        import_dir = CommonsBase_Remote__GitHub__0_1_1.path_join(project_root, "etc/dk/i")
      end
    end
    CommonsBase_Remote__GitHub__0_1_1.capture(
      request,
      local_program,
      {
      ...
      { cwd = bootstrap_root })
    return CommonsBase_Remote__GitHub__0_1_1.path_join(
      bootstrap_root,
      CommonsBase_Remote__GitHub__0_1_1.is_windows(request)
        and CommonsBase_Remote__GitHub__0_1_1.windows_relpath(program)
        or CommonsBase_Remote__GitHub__0_1_1.normalize_relpath(program))
  end
  return program
end
...
function CommonsBase_Remote__GitHub__0_1_1.host_tool_slot(request)
  if CommonsBase_Remote__GitHub__0_1_1.is_windows(request) then
    return "Release.Windows_x86_64"
  end
  local os_slot = "Linux"
  local uname_s = CommonsBase_Remote__GitHub__0_1_1.try_capture(
    request, "uname", { "-s" },
    { quiet = true, allowfailure = true, max_output_bytes = 4096 })
  if uname_s.code == "0" then
    local os_name = CommonsBase_Remote__GitHub__0_1_1.trim(
      CommonsBase_Remote__GitHub__0_1_1.first_line(uname_s.stdout))
    if os_name == "Darwin" then
      os_slot = "Darwin"
    end
  end
  local arch_slot = "x86_64"
  local uname_m = CommonsBase_Remote__GitHub__0_1_1.try_capture(
    request, "uname", { "-m" },
    { quiet = true, allowfailure = true, max_output_bytes = 4096 })
  if uname_m.code == "0" then
    local arch = CommonsBase_Remote__GitHub__0_1_1.trim(
      CommonsBase_Remote__GitHub__0_1_1.first_line(uname_m.stdout))
...
function CommonsBase_Remote__GitHub__0_1_1.tool_abs_path(request, bootstrap_root, relpath)
  return CommonsBase_Remote__GitHub__0_1_1.path_join(
    bootstrap_root,
    CommonsBase_Remote__GitHub__0_1_1.is_windows(request)
      and CommonsBase_Remote__GitHub__0_1_1.windows_relpath(relpath)
      or CommonsBase_Remote__GitHub__0_1_1.normalize_relpath(relpath))
end
...
function CommonsBase_Remote__GitHub__0_1_1.bootstrap_build_object(request, snapshot_dir, p, module_ver, subdir)
  local bootstrap_root = request.io.realpath(snapshot_dir)
  local local_program = CommonsBase_Remote__GitHub__0_1_1.local_dk0_program(request, snapshot_dir)
  local import_dir = "etc/dk/i"
  local project_root = p.project_root or ""
  ...
  CommonsBase_Remote__GitHub__0_1_1.capture(
    ...
    local_program,
    {
      ...
      "-s", CommonsBase_Remote__GitHub__0_1_1.host_tool_slot(request),
      "-d", subdir
    },
    { cwd = bootstrap_root })
...
function CommonsBase_Remote__GitHub__0_1_1.ensure_age(request, snapshot_dir, p)
  local exe = ""
  if CommonsBase_Remote__GitHub__0_1_1.is_windows(request) then
    exe = ".exe"
  end
  ...
  if probe.code ~= "0" then
    local bootstrap_root = CommonsBase_Remote__GitHub__0_1_1.bootstrap_build_object(
      request, snapshot_dir, p, "CommonsBase_Build.Age@1.3.1", subdir)
    return {
      age = CommonsBase_Remote__GitHub__0_1_1.tool_abs_path(request, bootstrap_root, age_rel),
      keygen = CommonsBase_Remote__GitHub__0_1_1.tool_abs_path(request, bootstrap_root, keygen_rel)
    }
  end
...
function CommonsBase_Remote__GitHub__0_1_1.ensure_gh(request, snapshot_dir, p)
  local exe = ""
  if CommonsBase_Remote__GitHub__0_1_1.is_windows(request) then
    exe = ".exe"
  end
  ...
  if probe.code ~= "0" then
    ...
    return CommonsBase_Remote__GitHub__0_1_1.tool_abs_path(request, bootstrap_root, gh_rel)
  end
  return gh_rel
end
...
function CommonsBase_Remote__GitHub__0_1_1.find_line_with_prefix(text, prefix)
  local start = 1
  ...
  while start <= len do
    ...
    if string.sub(line, 1, string.len(prefix)) == prefix then
      return CommonsBase_Remote__GitHub__0_1_1.trim(line)
    end
    if nl then
      start = nl + 1
    else
...
function CommonsBase_Remote__GitHub__0_1_1.gh_variable_get(request, p, ownerrepo, name)
  local r = CommonsBase_Remote__GitHub__0_1_1.try_capture(
    request, p.gh, { "variable", "get", name, "-R", ownerrepo },
    { quiet = true, allowfailure = true })
  if r.code == "0" then
    local v = CommonsBase_Remote__GitHub__0_1_1.trim(r.stdout or "")
    if v ~= "" then
      return v
    end
  end
...
function CommonsBase_Remote__GitHub__0_1_1.gh_variable_set(request, p, ownerrepo, name, value)
  CommonsBase_Remote__GitHub__0_1_1.capture(
...
function CommonsBase_Remote__GitHub__0_1_1.gh_secret_set(request, p, ownerrepo, coreutils, name, value)
  local env_rel = ".dk/r/c/t/c/dk-session-secret.env"
  CommonsBase_Remote__GitHub__0_1_1.write_project_text(
    request, coreutils, env_rel, name .. "=" .. value .. "\n", "0600")
  CommonsBase_Remote__GitHub__0_1_1.capture(
    request, p.gh, { "secret", "set", "-f", env_rel, "-R", ownerrepo },
    { quiet = true })
  CommonsBase_Remote__GitHub__0_1_1.try_capture(
    request, coreutils, { "rm", "-f", env_rel },
    { quiet = true, allowfailure = true })
end

function CommonsBase_Remote__GitHub__0_1_1.b64_encode_path(request, coreutils, path)
  local r = CommonsBase_Remote__GitHub__0_1_1.capture(
    request, coreutils, { "base64", "-w", "0", path }, { quiet = true })
  return CommonsBase_Remote__GitHub__0_1_1.trim(r.stdout or "")
end

function CommonsBase_Remote__GitHub__0_1_1.b64_decode_to_path(request, coreutils, b64, out_rel)
  local enc_rel = out_rel .. ".b64"
  CommonsBase_Remote__GitHub__0_1_1.write_project_text(request, coreutils, enc_rel, b64 .. "\n", "0644")
  local r = CommonsBase_Remote__GitHub__0_1_1.capture(
    request, coreutils, { "base64", "-d", enc_rel }, { quiet = true })
  CommonsBase_Remote__GitHub__0_1_1.write_project_text(request, coreutils, out_rel, r.stdout or "", "0644")
  CommonsBase_Remote__GitHub__0_1_1.try_capture(
...
function CommonsBase_Remote__GitHub__0_1_1.age_generate(request, age)
  local r = CommonsBase_Remote__GitHub__0_1_1.capture(request, age.keygen, {}, { quiet = true })
  local text = r.stdout or ""
  local recipient = nil
  local pub_marker = "# public key: "
  local pub_line = CommonsBase_Remote__GitHub__0_1_1.find_line_with_prefix(text, pub_marker)
  if pub_line then
    recipient = CommonsBase_Remote__GitHub__0_1_1.trim(string.sub(pub_line, string.len(pub_marker) + 1))
  end
  if not recipient then
    local err_marker = "Public key: "
    local err_line = CommonsBase_Remote__GitHub__0_1_1.find_line_with_prefix(r.stderr or "", err_marker)
    if err_line then
      recipient = CommonsBase_Remote__GitHub__0_1_1.trim(string.sub(err_line, string.len(err_marker) + 1))
    end
  end
  local secret = CommonsBase_Remote__GitHub__0_1_1.find_line_with_prefix(text, "AGE-SECRET-KEY-")
...
function CommonsBase_Remote__GitHub__0_1_1.ensure_session_recipient(request, ownerrepo, p, age, coreutils)
  local recip_var = CommonsBase_Remote__GitHub__0_1_1.gh_variable_get(request, p, ownerrepo, "dk_session_recipient")
  local sig_var = CommonsBase_Remote__GitHub__0_1_1.gh_variable_get(request, p, ownerrepo, "dk_session_recipient_sig")
  ...
  if recip_var and sig_var then
    CommonsBase_Remote__GitHub__0_1_1.write_project_text(request, coreutils, msg_rel, recip_var .. "\n", "0644")
    CommonsBase_Remote__GitHub__0_1_1.b64_decode_to_path(request, coreutils, sig_var, sig_rel)
  ...
  local kp = CommonsBase_Remote__GitHub__0_1_1.age_generate(request, age)
  CommonsBase_Remote__GitHub__0_1_1.gh_secret_set(request, p, ownerrepo, coreutils, "DK_SESSION_KEY", kp.secret)
  CommonsBase_Remote__GitHub__0_1_1.write_project_text(request, coreutils, msg_rel, kp.recipient .. "\n", "0644")
  ...
  local sig_b64 = CommonsBase_Remote__GitHub__0_1_1.b64_encode_path(request, coreutils, sig_rel)
  CommonsBase_Remote__GitHub__0_1_1.gh_variable_set(request, p, ownerrepo, "dk_session_recipient", kp.recipient)
  CommonsBase_Remote__GitHub__0_1_1.gh_variable_set(request, p, ownerrepo, "dk_session_recipient_sig", sig_b64)
  return kp.recipient
end
...
function CommonsBase_Remote__GitHub__0_1_1.enumerate_local_assets(request)
...
function CommonsBase_Remote__GitHub__0_1_1.stage_local_assets(request, snapshot_dir, p, coreutils)
  local locals = CommonsBase_Remote__GitHub__0_1_1.enumerate_local_assets(request)
...
function CommonsBase_Remote__GitHub__0_1_1.asset_manifest_text(items)
  local lines = {}
  local i = 1
  while items[i] do
...
function CommonsBase_Remote__GitHub__0_1_1.encrypt_and_upload_assets(request, ownerrepo, stage_tag, items, recipient, p, age)
  local stage_dir = ".dk/r/c/t/c/stage-assets"
  local i = 1
  while items[i] do
...
function CommonsBase_Remote__GitHub__0_1_1.normalize_relpath(path)
  path = tostring(path)
  local parts = {}
  local part_count = 0
  local i = 1
...
function CommonsBase_Remote__GitHub__0_1_1.windows_relpath(path)
  path = CommonsBase_Remote__GitHub__0_1_1.normalize_relpath(path)
  local parts = {}
  local part_count = 0
  local i = 1
  while i <= string.len(path) do
...
function CommonsBase_Remote__GitHub__0_1_1.ends_with(text, suffix)
...
function CommonsBase_Remote__GitHub__0_1_1.sort_strings(values)
  local i = 2
  while values[i] do
...
function CommonsBase_Remote__GitHub__0_1_1.extract_14_digit_timestamp(text)
  text = tostring(text or "")
  local start = 1
...
function CommonsBase_Remote__GitHub__0_1_1.synthetic_14_digit_timestamp(request)
...
function CommonsBase_Remote__GitHub__0_1_1.json_string_field(json_text, field_name)
...
function CommonsBase_Remote__GitHub__0_1_1.write_project_text(request, coreutils, dest_rel, content, mode)
  local temp_rel =
    "remote-github/generated-" ..
    CommonsBase_Remote__GitHub__0_1_1.basename(dest_rel) ..
    "-" .. request.rule.generatesymbol() .. ".txt"
  local temp_abs = CommonsBase_Remote__GitHub__0_1_1.write_text(request, temp_rel, content)
  CommonsBase_Remote__GitHub__0_1_1.install_project_file(request, coreutils, temp_abs, dest_rel, mode)
end

function CommonsBase_Remote__GitHub__0_1_1.install_project_file(request, coreutils, source_abs, dest_rel, mode)
  local dest_dir = CommonsBase_Remote__GitHub__0_1_1.dirname(dest_rel)
  if dest_dir and dest_dir ~= "" and dest_dir ~= "." then
    CommonsBase_Remote__GitHub__0_1_1.spawn(request, coreutils, { "mkdir", "-p", dest_dir })
  end
  CommonsBase_Remote__GitHub__0_1_1.spawn(request, coreutils, { "rm", "-f", dest_rel })
  CommonsBase_Remote__GitHub__0_1_1.spawn(request, coreutils, { "cp", "-f", source_abs, dest_rel })
end

function CommonsBase_Remote__GitHub__0_1_1.copy_snapshot_file_to_project(request, coreutils, source_file, dest_rel)
  local source_abs = request.io.realpath(source_file)
  CommonsBase_Remote__GitHub__0_1_1.install_project_file(request, coreutils, source_abs, dest_rel)
end

function CommonsBase_Remote__GitHub__0_1_1.copy_project_dir_to_commit(request, dir, p, copied, seen)
  local entries = request.io.list(dir, "all")
  local i = 1
  while entries[i] do
    local entry = entries[i]
    if request.io.isdir(entry) then
      CommonsBase_Remote__GitHub__0_1_1.copy_project_dir_to_commit(request, entry, p, copied, seen)
    elseif request.io.isfile(entry) then
      local source_rel = CommonsBase_Remote__GitHub__0_1_1.normalize_relpath(request.io.realpath(entry, { relative = 1 }))
      local source_name = CommonsBase_Remote__GitHub__0_1_1.basename(source_rel)
      local workspace_name = CommonsBase_Remote__GitHub__0_1_1.basename(CommonsBase_Remote__GitHub__0_1_1.normalize_relpath(p.workspace))
      local rel = nil
      local dest_rel = nil
      ...
      if dest_rel then
        CommonsBase_Remote__GitHub__0_1_1.copy_snapshot_file_to_project(request, p.coreutils, entry, dest_rel)
        if rel then
...
function CommonsBase_Remote__GitHub__0_1_1.ensure_control_tree(request, coreutils)
  CommonsBase_Remote__GitHub__0_1_1.write_project_text(request, coreutils, ".dk/r/.gitignore", "*\n", "0644")
  CommonsBase_Remote__GitHub__0_1_1.write_project_text(request, coreutils, ".dk/r/.hgignore", "syntax: glob\n*\n", "0644")
end

function CommonsBase_Remote__GitHub__0_1_1.collect_relative_files(request, dir, root, out, rel_prefix)
  local entries = request.io.list(dir, "all")
  local i = 1
  while entries[i] do
    local entry = entries[i]
    if request.io.isdir(entry) then
      CommonsBase_Remote__GitHub__0_1_1.collect_relative_files(
        request,
        entry,
        root,
        out,
...
function CommonsBase_Remote__GitHub__0_1_1.prepare_commit_repo_inputs(request, snapshot_dir, p, coreutils)
  local copied = {}
  local seen = {}
  ...
  CommonsBase_Remote__GitHub__0_1_1.ensure_commit_repo_gitignore(request, coreutils, ".dk/r/c")
  CommonsBase_Remote__GitHub__0_1_1.copy_project_dir_to_commit(request, snapshot_dir, p, copied, seen)
  return copied
end

function CommonsBase_Remote__GitHub__0_1_1.write_windows_wrapper(request, wrapper_name, body)
  return CommonsBase_Remote__GitHub__0_1_1.write_text(
    request,
    "remote-github/bin/" .. wrapper_name .. ".cmd",
    "@echo off\r\n" .. body .. "\r\n")
end

function CommonsBase_Remote__GitHub__0_1_1.wrap_windows_program(request, wrapper_name, target)
  local body = "if exist \"" .. target .. "\" goto found\r\n" ..
    ...
    "\"" .. target .. "\" %*\r\n" ..
    "exit /b %ERRORLEVEL%"
  return CommonsBase_Remote__GitHub__0_1_1.write_windows_wrapper(request, wrapper_name, body)
end

function CommonsBase_Remote__GitHub__0_1_1.write_windows_path_wrapper(request, wrapper_name, command_name, fallback_dirs)
  local lines = {
    "setlocal"
  }
  local i = 1
  ...
  return CommonsBase_Remote__GitHub__0_1_1.write_windows_wrapper(request, wrapper_name, table.concat(lines, "\r\n"))
end

function CommonsBase_Remote__GitHub__0_1_1.resolve_programs(request, p)
  ...
  if p.gh_user_supplied then
    local gh_probe = CommonsBase_Remote__GitHub__0_1_1.try_capture(
    ...
    if gh_probe.code ~= "0" then
      if string.find(p.gh, " ", 1, true) then
        local wrapped = CommonsBase_Remote__GitHub__0_1_1.wrap_windows_program(request, "resolve-gh", p.gh)
        local wrapped_probe = CommonsBase_Remote__GitHub__0_1_1.try_capture(
  ...
  local git_probe = CommonsBase_Remote__GitHub__0_1_1.try_capture(
    request, p.git, { "--version" }, { quiet = true, allowfailure = true, max_output_bytes = 4096 })
  if git_probe.code ~= "0" then
    if p.git == "git" then
      local wrapped = CommonsBase_Remote__GitHub__0_1_1.write_windows_path_wrapper(
        request,
        "resolve-git",
        "git",
    ...
    elseif string.find(p.git, " ", 1, true) then
      local wrapped = CommonsBase_Remote__GitHub__0_1_1.wrap_windows_program(request, "resolve-git", p.git)
      local wrapped_probe = CommonsBase_Remote__GitHub__0_1_1.try_capture(
        request, wrapped, { "--version" }, { quiet = true, allowfailure = true, max_output_bytes = 4096 })
      if wrapped_probe.code == "0" then
        p.git = wrapped
...
function CommonsBase_Remote__GitHub__0_1_1.read_text(request, path)
...
function CommonsBase_Remote__GitHub__0_1_1.safe_name(s)
...
function CommonsBase_Remote__GitHub__0_1_1.ownerrepo(repo)
...
function CommonsBase_Remote__GitHub__0_1_1.now_utc(request, p, coreutils)
  ...
  if coreutils then
    local dated = CommonsBase_Remote__GitHub__0_1_1.try_capture(
      request, coreutils, { "date", "-u", "+%Y%m%d%H%M%S" },
      { quiet = true, allowfailure = true })
    local ts = CommonsBase_Remote__GitHub__0_1_1.extract_14_digit_timestamp(dated.stdout)
    if ts then
      return ts
    end
  end
  local result = CommonsBase_Remote__GitHub__0_1_1.try_capture(
    ...
    { quiet = true, allowfailure = true })
  local timestamp = CommonsBase_Remote__GitHub__0_1_1.extract_14_digit_timestamp(result.stdout)
  if not timestamp then
    timestamp = CommonsBase_Remote__GitHub__0_1_1.extract_14_digit_timestamp(result.stderr)
  end
  if not timestamp then
    ...
    timestamp = CommonsBase_Remote__GitHub__0_1_1.synthetic_14_digit_timestamp(request)
  end
  assert(timestamp, "Could not derive a timestamp")
  return timestamp
end

function CommonsBase_Remote__GitHub__0_1_1.workflow_yaml(session, keep)
  local template = table.concat({
    "# Generated by CommonsBase_Remote.GitHub@0.1.1.",
  ...
  template = CommonsBase_Remote__GitHub__0_1_1.replace_all(template, "__SESSION__", tostring(session))
  template = CommonsBase_Remote__GitHub__0_1_1.replace_all(template, "__KEEP__", tostring(keep))
  return template
end

function CommonsBase_Remote__GitHub__0_1_1.wait_release(request, ownerrepo, tag, p)
  local attempt = 1
  while attempt <= 12 do
    ...
    local result = CommonsBase_Remote__GitHub__0_1_1.try_capture(
      ...
      { quiet = true })
    local url = CommonsBase_Remote__GitHub__0_1_1.json_string_field(result.stdout, "url") or ""
...
function CommonsBase_Remote__GitHub__0_1_1.workflow_base_name(workflow)
...
function CommonsBase_Remote__GitHub__0_1_1.find_workflow_run(json_text)
  local segment = json_text or ""
  local status = CommonsBase_Remote__GitHub__0_1_1.json_string_field(segment, "status")
  local conclusion = CommonsBase_Remote__GitHub__0_1_1.json_string_field(segment, "conclusion") or ""
  local url = CommonsBase_Remote__GitHub__0_1_1.json_string_field(segment, "url")
...
function CommonsBase_Remote__GitHub__0_1_1.find_run_by_sha(json_text, head_sha)
  ...
  while pos <= string.len(text) do
    ...
    if string.find(chunk, needle, 1, true) then
      return {
        status = CommonsBase_Remote__GitHub__0_1_1.json_string_field(chunk, "status"),
        conclusion = CommonsBase_Remote__GitHub__0_1_1.json_string_field(chunk, "conclusion") or "",
        url = CommonsBase_Remote__GitHub__0_1_1.json_string_field(chunk, "url") or ""
      }
    end
  end
  return nil
...
function CommonsBase_Remote__GitHub__0_1_1.wait_workflow(request, ownerrepo, branch, workflow, head_sha, p)
  ...
  while attempt <= 60 do
    ...
    local result = CommonsBase_Remote__GitHub__0_1_1.try_capture(
      ...
      { quiet = true })
    if result.code == "0" then
      local found = CommonsBase_Remote__GitHub__0_1_1.find_run_by_sha(result.stdout, head_sha)
      ...
      local msg = CommonsBase_Remote__GitHub__0_1_1.trim(result.stderr or "")
      if msg ~= "" then
...
function CommonsBase_Remote__GitHub__0_1_1.ensure_repo(request, ownerrepo, p)
  CommonsBase_Remote__GitHub__0_1_1.capture(request, p.gh, { "auth", "status" }, { quiet = true })
  if p.create_repo then
    local created = CommonsBase_Remote__GitHub__0_1_1.try_capture(
    ...
    assert(false, "Could not create private repository " .. ownerrepo .. ": " .. CommonsBase_Remote__GitHub__0_1_1.trim(created.stderr))
  end
  local view = CommonsBase_Remote__GitHub__0_1_1.try_capture(
...
function CommonsBase_Remote__GitHub__0_1_1.commit_repo_root(request, commit_dir, p)
  local attempt = 1
  while attempt <= 3 do
    local actual_root = CommonsBase_Remote__GitHub__0_1_1.try_capture(
      request,
      p.git,
      { "-C", commit_dir, "rev-parse", "--show-toplevel" },
      { quiet = true, allowfailure = true })
    if actual_root.code == "0" then
      return CommonsBase_Remote__GitHub__0_1_1.normalize_relpath(
        CommonsBase_Remote__GitHub__0_1_1.trim(actual_root.stdout or ""))
    end
    if attempt < 3 then
...
function CommonsBase_Remote__GitHub__0_1_1.commit_repo_is_isolated(request, commit_dir, p)
  local commit_git_dir = commit_dir .. "/.git"
  if not (request.io.isdir(commit_git_dir) or request.io.isfile(commit_git_dir)) then
    return false, nil
  end
  local expected_root = CommonsBase_Remote__GitHub__0_1_1.normalize_relpath(
    request.io.realpath(commit_dir))
  local actual_root =
    CommonsBase_Remote__GitHub__0_1_1.commit_repo_root(request, commit_dir, p)
  return actual_root == expected_root, actual_root
end

function CommonsBase_Remote__GitHub__0_1_1.ensure_commit_repo(request, ownerrepo, commit_dir, coreutils, p)
  local commit_git_dir = commit_dir .. "/.git"
  if request.io.isdir(commit_git_dir) or request.io.isfile(commit_git_dir) then
    CommonsBase_Remote__GitHub__0_1_1.spawn(
      request,
      coreutils,
  ...
  CommonsBase_Remote__GitHub__0_1_1.capture(
    ...
    p.git,
    { "-C", commit_dir, "init" })
  local add_origin = CommonsBase_Remote__GitHub__0_1_1.try_capture(
    request,
    p.git,
  ...
  CommonsBase_Remote__GitHub__0_1_1.capture(request, p.git, { "-C", commit_dir, "config", "user.name", "dk remote session" }, { quiet = true })
  CommonsBase_Remote__GitHub__0_1_1.capture(request, p.git, { "-C", commit_dir, "config", "user.email", "dk-remote-session@users.noreply.github.com" }, { quiet = true })
  CommonsBase_Remote__GitHub__0_1_1.try_capture(request, p.git, { "-C", commit_dir, "fetch", "origin", "--prune", "--tags" }, { quiet = true, allowfailure = true })
end

function CommonsBase_Remote__GitHub__0_1_1.commit_repo_gitignore_text()
  return table.concat({
...
function CommonsBase_Remote__GitHub__0_1_1.commit_repo_gitattributes_text()
...
function CommonsBase_Remote__GitHub__0_1_1.ensure_commit_repo_gitignore(request, coreutils, commit_dir)
  CommonsBase_Remote__GitHub__0_1_1.write_project_text(
    request,
    coreutils,
    commit_dir .. "/.gitignore",
    CommonsBase_Remote__GitHub__0_1_1.commit_repo_gitignore_text(),
    "0644")
  CommonsBase_Remote__GitHub__0_1_1.write_project_text(
    request,
    coreutils,
    commit_dir .. "/.gitattributes",
    CommonsBase_Remote__GitHub__0_1_1.commit_repo_gitattributes_text(),
    "0644")
end

function CommonsBase_Remote__GitHub__0_1_1.reset_commit_repo_worktree(request, commit_dir, p)
  CommonsBase_Remote__GitHub__0_1_1.try_capture(
    request,
    p.git,
...
function CommonsBase_Remote__GitHub__0_1_1.stash_commit_repo_changes(request, commit_dir, p)
  local status = CommonsBase_Remote__GitHub__0_1_1.try_capture(
    request,
    p.git,
    { "-C", commit_dir, "status", "--porcelain=v1", "--untracked-files=all" },
    { quiet = true, allowfailure = true })
  if CommonsBase_Remote__GitHub__0_1_1.trim(status.stdout) == "" then
    return
  end
  CommonsBase_Remote__GitHub__0_1_1.capture(
    request,
...
function CommonsBase_Remote__GitHub__0_1_1.ensure_branches(request, commit_dir, sessions, p)
  CommonsBase_Remote__GitHub__0_1_1.reset_commit_repo_worktree(request, commit_dir, p)
  local has_root = CommonsBase_Remote__GitHub__0_1_1.try_capture(
    request, p.git, { "-C", commit_dir, "ls-remote", "--heads", "origin", "dk-session-root" },
    { quiet = true })
  if CommonsBase_Remote__GitHub__0_1_1.trim(has_root.stdout) == "" then
    CommonsBase_Remote__GitHub__0_1_1.try_capture(request, p.git, { "-C", commit_dir, "checkout", "--orphan", "dk-session-root" })
    CommonsBase_Remote__GitHub__0_1_1.capture(request, p.git, { "-C", commit_dir, "commit", "--allow-empty", "-m", "Create dk-session root" })
    CommonsBase_Remote__GitHub__0_1_1.try_capture(request, p.git, { "-C", commit_dir, "branch", "-M", "dk-session-root" })
    CommonsBase_Remote__GitHub__0_1_1.capture(request, p.git, { "-C", commit_dir, "push", "-u", "origin", "HEAD:refs/heads/dk-session-root" })
  else
    CommonsBase_Remote__GitHub__0_1_1.capture(request, p.git, { "-C", commit_dir, "checkout", "-B", "dk-session-root", "origin/dk-session-root" })
  end
  local i = 1
  while i <= sessions do
    local branch = "dk-session-" .. tostring(i)
    local has_branch = CommonsBase_Remote__GitHub__0_1_1.try_capture(
      request, p.git, { "-C", commit_dir, "ls-remote", "--heads", "origin", branch },
      { quiet = true })
    if CommonsBase_Remote__GitHub__0_1_1.trim(has_branch.stdout) == "" then
      CommonsBase_Remote__GitHub__0_1_1.capture(request, p.git, { "-C", commit_dir, "checkout", "-B", branch, "origin/dk-session-root" })
      CommonsBase_Remote__GitHub__0_1_1.capture(request, p.git, { "-C", commit_dir, "push", "-u", "origin", "HEAD:refs/heads/" .. branch })
    end
    i = i + 1
  end
end

function CommonsBase_Remote__GitHub__0_1_1.select_session(request, ownerrepo, sessions, p)
  local order = {}
  ...
  while order[i] do
    ...
    local running = CommonsBase_Remote__GitHub__0_1_1.try_capture(
      request,
      p.gh,
      {
    ...
    if running.code == "0" then
      ...
      found = CommonsBase_Remote__GitHub__0_1_1.find_workflow_run(combined)
    end
...
function CommonsBase_Remote__GitHub__0_1_1.write_checksum_manifest(request, clone_root, rel_paths, manifest_rel, coreutils)
  local lines = {}
  local i = 1
  while rel_paths[i] do
    local meta = request.ui.checksum { path = CommonsBase_Remote__GitHub__0_1_1.path_join(clone_root, rel_paths[i]) }
    table.insert(lines, meta.sha256 .. " *" .. rel_paths[i])
    i = i + 1
  end
  CommonsBase_Remote__GitHub__0_1_1.sort_strings(lines)
  CommonsBase_Remote__GitHub__0_1_1.write_project_text(
    request,
    coreutils,
    CommonsBase_Remote__GitHub__0_1_1.path_join(clone_root, manifest_rel),
    table.concat(lines, "\n") .. "\n",
    "0644")
end

function CommonsBase_Remote__GitHub__0_1_1.sign_file(request, clone_root, message_rel, signature_rel)
  local signed, msg = request.ui.signify {
    operation = "sign",
    secret_key = CommonsBase_Remote__GitHub__0_1_1.path_join(clone_root, "t/k/build.sec"),
    message = CommonsBase_Remote__GitHub__0_1_1.path_join(clone_root, message_rel),
    signature = CommonsBase_Remote__GitHub__0_1_1.path_join(clone_root, signature_rel)
  }
  assert(signed, "Could not sign " .. message_rel .. ": " .. tostring(msg))
end

function CommonsBase_Remote__GitHub__0_1_1.download_result(request, ownerrepo, tag, timestamp, p)
  ...
  CommonsBase_Remote__GitHub__0_1_1.capture(request, p.gh, {
...
function CommonsBase_Remote__GitHub__0_1_1.basename(path)
  local i = string.len(path)
  while i >= 1 do
    local ch = string.sub(path, i, i)
    if ch == "/" or ch == "\\" then
...
function CommonsBase_Remote__GitHub__0_1_1.read_named_file(request, dir, name)
  local entries = request.io.list(dir, "all")
  local i = 1
  while entries[i] do
    local entry = entries[i]
    local rel = request.io.realpath(entry, { relative = 1 })
    if request.io.isfile(entry) and CommonsBase_Remote__GitHub__0_1_1.basename(rel) == name then
...
function CommonsBase_Remote__GitHub__0_1_1.orchestrate_submit(request, p)
  local ownerrepo = CommonsBase_Remote__GitHub__0_1_1.ownerrepo(p.repo)
  CommonsBase_Remote__GitHub__0_1_1.resolve_programs(request, p)
  ...
  local coreutils = CommonsBase_Remote__GitHub__0_1_1.ensure_coreutils(request, snapshot_dir, p)
  if not p.gh_user_supplied then
    p.gh = CommonsBase_Remote__GitHub__0_1_1.ensure_gh(request, snapshot_dir, p)
  end
  local age = CommonsBase_Remote__GitHub__0_1_1.ensure_age(request, snapshot_dir, p)
  local commit_dir = ".dk/r/c"
  local timestamp = p.timestamp
  if timestamp == "19700101000000" then
    timestamp = CommonsBase_Remote__GitHub__0_1_1.now_utc(request, p, coreutils)
  end

  CommonsBase_Remote__GitHub__0_1_1.ensure_repo(request, ownerrepo, p)
  CommonsBase_Remote__GitHub__0_1_1.ensure_control_tree(request, coreutils)
  CommonsBase_Remote__GitHub__0_1_1.ensure_commit_repo(request, ownerrepo, commit_dir, coreutils, p)
  CommonsBase_Remote__GitHub__0_1_1.ensure_branches(request, commit_dir, p.sessions, p)
  CommonsBase_Remote__GitHub__0_1_1.stash_commit_repo_changes(request, commit_dir, p)

  local selected = CommonsBase_Remote__GitHub__0_1_1.select_session(request, ownerrepo, p.sessions, p)
  ...
  CommonsBase_Remote__GitHub__0_1_1.capture(request, p.git, { "-C", commit_dir, "checkout", "-B", branch, "origin/" .. branch })
  CommonsBase_Remote__GitHub__0_1_1.capture(request, p.git, { "-C", commit_dir, "pull", "--rebase", "origin", branch })
  ...
  local copied = CommonsBase_Remote__GitHub__0_1_1.prepare_commit_repo_inputs(request, snapshot_dir, p, coreutils)
  ...
  local staged_assets = CommonsBase_Remote__GitHub__0_1_1.stage_local_assets(request, snapshot_dir, p, coreutils)
  ...
  if staged_assets[1] then
    recipient = CommonsBase_Remote__GitHub__0_1_1.ensure_session_recipient(request, ownerrepo, p, age, coreutils)
  end
  ...
  CommonsBase_Remote__GitHub__0_1_1.write_project_text(request, coreutils, commit_dir .. "/" .. audit_rel, timestamp .. "Z " .. p.commandvsl .. "\n", "0644")
  CommonsBase_Remote__GitHub__0_1_1.write_project_text(
    request,
    coreutils,
    commit_dir .. "/" .. argv_rel,
    CommonsBase_Remote__GitHub__0_1_1.base64_lines_text(p.argv),
    "0644")
  CommonsBase_Remote__GitHub__0_1_1.write_project_text(
    request,
    coreutils,
    commit_dir .. "/" .. workflow_rel,
    CommonsBase_Remote__GitHub__0_1_1.workflow_yaml(selected, p.retention),
    "0644")
  CommonsBase_Remote__GitHub__0_1_1.write_checksum_manifest(request, commit_dir, { audit_rel, argv_rel, workflow_rel }, stage_index_rel, coreutils)
  CommonsBase_Remote__GitHub__0_1_1.sign_file(request, commit_dir, stage_index_rel, stage_sig_rel)
  CommonsBase_Remote__GitHub__0_1_1.capture(
    request,
    p.git,
    {
  ...
  CommonsBase_Remote__GitHub__0_1_1.capture(request, p.git, { "-C", commit_dir, "commit", "-m", "dk remote " .. timestamp .. " stage" })
  -- (lua-ml cannot index a call result directly, so use a temp variable)
  local stage_head = CommonsBase_Remote__GitHub__0_1_1.capture(request, p.git, { "-C", commit_dir, "rev-parse", "HEAD" })
  local stage_sha = CommonsBase_Remote__GitHub__0_1_1.trim(stage_head.stdout)
  CommonsBase_Remote__GitHub__0_1_1.capture(request, p.git, { "-C", commit_dir, "tag", stage_tag })
  CommonsBase_Remote__GitHub__0_1_1.capture(request, p.git, { "-C", commit_dir, "push", "origin", branch })
  CommonsBase_Remote__GitHub__0_1_1.capture(request, p.git, { "-C", commit_dir, "push", "origin", stage_tag })
  CommonsBase_Remote__GitHub__0_1_1.wait_workflow(request, ownerrepo, branch, workflow, stage_sha, p)
  CommonsBase_Remote__GitHub__0_1_1.wait_release(request, ownerrepo, stage_tag, p)
  ...
  if staged_assets[1] then
    CommonsBase_Remote__GitHub__0_1_1.encrypt_and_upload_assets(request, ownerrepo, stage_tag, staged_assets, recipient, p, age)
    CommonsBase_Remote__GitHub__0_1_1.write_project_text(
      request, coreutils, commit_dir .. "/" .. assets_rel,
      CommonsBase_Remote__GitHub__0_1_1.asset_manifest_text(staged_assets), "0644")
  end
  ...
  CommonsBase_Remote__GitHub__0_1_1.write_checksum_manifest(request, commit_dir, manifest_paths, "INDEX", coreutils)
  CommonsBase_Remote__GitHub__0_1_1.sign_file(request, commit_dir, "INDEX", "INDEX.sig")
  CommonsBase_Remote__GitHub__0_1_1.capture(request, p.git, { "-C", commit_dir, "add", "-A" })
  CommonsBase_Remote__GitHub__0_1_1.capture(request, p.git, { "-C", commit_dir, "commit", "-m", "dk remote " .. timestamp .. " exec" })
  local exec_head = CommonsBase_Remote__GitHub__0_1_1.capture(request, p.git, { "-C", commit_dir, "rev-parse", "HEAD" })
  local exec_sha = CommonsBase_Remote__GitHub__0_1_1.trim(exec_head.stdout)
  CommonsBase_Remote__GitHub__0_1_1.capture(request, p.git, { "-C", commit_dir, "tag", exec_tag })
  CommonsBase_Remote__GitHub__0_1_1.capture(request, p.git, { "-C", commit_dir, "push", "origin", branch })
  CommonsBase_Remote__GitHub__0_1_1.capture(request, p.git, { "-C", commit_dir, "push", "origin", exec_tag })
  CommonsBase_Remote__GitHub__0_1_1.wait_workflow(request, ownerrepo, branch, workflow, exec_sha, p)
  CommonsBase_Remote__GitHub__0_1_1.wait_release(request, ownerrepo, exec_tag, p)
  local result_dir = CommonsBase_Remote__GitHub__0_1_1.download_result(request, ownerrepo, exec_tag, timestamp, p)
  ...
  CommonsBase_Remote__GitHub__0_1_1.present_result(request, coreutils, result_dir)
  return {
    submit = {
      values = {
...
function CommonsBase_Remote__GitHub__0_1_1.read_path(request, coreutils, path)
  ...
  local r = CommonsBase_Remote__GitHub__0_1_1.capture(request, coreutils, { "cat", path }, { quiet = true })
  return r.stdout or ""
end
...
function CommonsBase_Remote__GitHub__0_1_1.sanitize_output(text)
  local out = {}
  local i = 1
  local len = string.len(text)
  while i <= len do
...
function CommonsBase_Remote__GitHub__0_1_1.present_result(request, coreutils, result_file)
  -- Parse the combined result file produced on the runner:
  --   DKEXIT <code>\n<<<DKSTDOUT>>>\n<stdout>\n<<<DKSTDERR>>>\n<stderr>
  local text = CommonsBase_Remote__GitHub__0_1_1.read_path(request, coreutils, result_file)
  local exit_code = 0
  local first = CommonsBase_Remote__GitHub__0_1_1.first_line(text)
  if first and string.sub(first, 1, 7) == "DKEXIT " then
    exit_code = tonumber(CommonsBase_Remote__GitHub__0_1_1.trim(string.sub(first, 8))) or 0
  end
  ...
  if stdout_text and stdout_text ~= "" then
    ...
    print(CommonsBase_Remote__GitHub__0_1_1.sanitize_output(stdout_text))
  end
  if stderr_text and stderr_text ~= "" then
    print("----- remote stderr (runner-attested exit code; content is remote) -----")
    print(CommonsBase_Remote__GitHub__0_1_1.sanitize_output(stderr_text))
  end
  assert(exit_code == 0, "Remote command exited with code " .. tostring(exit_code))
end

function rules.F_DryRunPlan(command, request)
  local p = CommonsBase_Remote__GitHub__0_1_1.parse_common_args(request)
  local path = "dry-run.txt"
  if not p.dryrun then
    error("Only `dry_run=true` is implemented for CommonsBase_Remote.GitHub.F_DryRunPlan@0.1.1")
  end
  if command == "declareoutput" then
    return {
      declareoutput = {
        return_asset = {
          id = "CommonsBase_Remote.GitHub.F_DryRunPlan.Output." .. request.rule.generatesymbol() .. "@0.1.1",
          path = path
        }
      }
    }
  elseif command == "submit" then
    local file = request.io.open(path, "w")
    CommonsBase_Remote__GitHub__0_1_1.write_plan(request, file, p, "F_DryRunPlan")
...
function uirules.Run(command, request, continue_)
  ...
  if command == "submit" then
    ...
    elseif continue_ == "orchestrate" then
      return CommonsBase_Remote__GitHub__0_1_1.orchestrate_submit(request, p)
    else
  ...
  elseif command == "ui" then
    if p.dryrun then
      ...
      CommonsBase_Remote__GitHub__0_1_1.write_plan(request, file, p, "Run")
      request.io.close(file)
