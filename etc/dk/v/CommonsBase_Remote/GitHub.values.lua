...
function CommonsBase_Remote__GitHub__0_2_0.ensure_session_recipient(request, ownerrepo, p, age, coreutils)
  ...
  -- Sign with the host build key (no secret_key): the process signing identity
  -- behind t/k/build.pub, which the runner verifies with the committed public
  -- key. The secret is never copied into the project tree.
  ...
  while entries[i] do
    ...
    elseif request.io.isfile(entry) then
      ...
      elseif source_name == "build.pub" then
        ...
        -- globbed or copied (dk0 excludes it from request.ui.glob, F-1);
        -- signing uses the host build key directly (see sign_file).
        rel = "t/k/build.pub"
...
function CommonsBase_Remote__GitHub__0_2_0.sign_file(request, clone_root, message_rel, signature_rel)
  -- Sign with the host build key: request.ui.signify falls back to the process
  -- signing identity (the same key behind t/k/build.pub) when no secret_key is
  -- given. The secret is never copied into the project tree, so nothing needs to
  -- glob t/k/build.sec (dk0 excludes the secret from request.ui.glob, F-1).
  ...
  if command == "submit" then
    ...
    if continue_ == nil or continue_ == "start" then
      local patterns = {
        ...
        "etc/dk/i/**",
        "etc/dk/v/**",
        -- Only the PUBLIC key is snapshotted (committed so the runner can verify
        -- signed manifests). The signing secret build.sec is never globbed;
        -- signing uses the host build key directly (see sign_file), and dk0
        -- excludes build.sec from request.ui.glob (F-1).
        "t/k/build.pub"
      }
